...
#
# 这里将介绍如何使用 `Plots.jl`。
# 如果还没安装 `Plots.jl`，你需要通过包管理器（package manager）来安装它，Julia会在你第一次导入它的时候将它预编译：
# ------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------
# Plots 通过同样的接口可以支持多种后端——指的是实际执行绘制的库。
# 刚开始，我们先使用 GR 后端。通过调用 `gr()` 来选择 GR 后端：
# ------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------
# 注意到第二幅图和第一幅的区别！
# 既然 Jupyter notebook 有酷炫的绘图能力，使用文本符号来绘图就显得有些傻了。
# 但其实这在终端中进行快速简陋的可视化是很实用的。
# ------------------------------------------------------------------------------------------
...
# x = -10:10
# ```
# 要绘制 y 随 x 变化曲线，其中 $y = x^2$。你可能需要先把后端切换回去。
# ------------------------------------------------------------------------------------------
...
#
# 提示：你可以参考 Plots 的帮助文档 [GR - Plots](http://docs.juliaplots.org/latest/examples/gr/#magic-
# grid-argument)
# ------------------------------------------------------------------------------------------
